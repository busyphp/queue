...
use BusyPHP\queue\event\JobExceptionOccurred;
use BusyPHP\queue\event\JobFailed;
use BusyPHP\queue\event\JobProcessed;
use BusyPHP\queue\event\JobProcessing;
use BusyPHP\queue\event\WorkerStopping;
use BusyPHP\queue\exception\MaxAttemptsExceededException;
use Throwable;
...
class Worker
{
    ...
    protected function markJobAsFailedIfAlreadyExceedsMaxAttempts($connection, $job, $maxTries)
    {
        ...
        $this->failJob($connection, $job, $e = new MaxAttemptsExceededException($job->getName() . ' has been attempted too many times or run too long. The job may have previously timed out.'));
